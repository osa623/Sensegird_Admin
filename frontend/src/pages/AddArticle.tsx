...
const createArticle = async (articleData: CreateArticleDTO): Promise<Article> => {
  try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Your article "${formData.title}" has been published successfully.`,
      });
      
      navigate("/dashboard");
    } catch (error: any) {
      console.error("Error publishing article:", error);
      let errorMessage = "Failed to publish article. Please check console for details.";
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6 animate-fade-in p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Add New Article</h1>
          <p className="text-sm sm:text-base text-muted-foreground">Create and publish your content</p>
        </div>
      </div>
